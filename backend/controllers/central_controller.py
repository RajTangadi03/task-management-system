...
async def create_user(data: userData, current_user: dict = Depends(oauth2.get_current_user)):
...
async def create_task(data: taskData, current_user: dict = Depends(oauth2.get_current_user)):
...
async def update_task(data: taskUpdateData, id: int, current_user: dict = Depends(oauth2.get_current_user)):
    if current_user["role"] not in ["admin", "manager"]:
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="Insufficient permissions")
...
async def delete_task(id: int, current_user: dict = Depends(oauth2.get_current_user)):
    if current_user["role"] not in ["admin", "manager"]:
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="Insufficient permissions")
...
async def update_user(data: userData, id: int, current_user: dict = Depends(oauth2.get_current_user)):
    if current_user["role"] not in ["admin", "manager"]:
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="Insufficient permissions")
